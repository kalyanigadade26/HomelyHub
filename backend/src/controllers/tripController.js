...
// send the info to our ai trip planner
// calculate the budget
// search in mongodb for suitable properties
// send both AI trip plan + matchong properties back to the frontend
    ...
    try{
        ...
        if (!destination || !budget || !days || !people){
            return res.status(400).json({
                status:"fail",
                message: "Please fill the in the destination, budget, days, people"
            })
        }
...
const writeDescription = async (req , res) =>{
    try{
        ...
        res.status(500).json({
            status:"fail",
            message: "could not genertae a description"
        })
